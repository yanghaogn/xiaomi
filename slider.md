...
4. agent
 1. The core provider在container上部署最小的agent, 然后, as the agent checks in to the agent provider's REST API, executes commands issued to it，这些命令包括：从hdfs下载包、解压这些包，然后按照模板并使用真实的配置通过python脚本启动
5. 
